...
**1. Установите API-ключ.** Сгенерируйте надёжный случайный ключ и задайте `KOKORO_API_KEY` в `env`-файле. После этого все API-запросы должны содержать `Authorization: Bearer <ключ>`.
